...
export default class jsTPS {
  constructor(app) {
    ...
    //! PART 7 - GIVE TPS ITS OWN APP ISNTANCE TO MAKE DISABLING/ENABLING THE UNDO/REDO BUTTONS EASIER
    this.app = app;
  }
  ...
  doTransaction() {
    ...
    // ! PART 7 - Call the other two methods after a transaction is done
  ...
  // ! PART 7 - These two methods control the buttons for redo-ing and undo-ing, better to put them here than in the controller file for every transaction
  enableOrDisableRedoButton() {
    if (this.hasTransactionToRedo()) {
      this.app.enableButton("redo-button");
    } else {
      this.app.disableButton("redo-button");
    }
  }

  enableOrDisableUndoButton() {
    if (this.hasTransactionToUndo()) {
      this.app.enableButton("undo-button");
    } else {
      this.app.disableButton("undo-button");
    }
  }

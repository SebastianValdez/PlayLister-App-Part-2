...
export default class PlaylistCards extends React.Component {
  render() {
    const {
      currentList,
      moveSongCallback,
      markIndexForSongDeletion,
      markIndexForSongEditing,
    } = this.props;
    if (currentList === null) {
        ...
        <div id="playlist-cards">
          {currentList.songs.map((song, index) => (
            <SongCard
              ...
              markIndexForSongDeletionCallback={markIndexForSongDeletion}
              markIndexForSongEditingCallback={markIndexForSongEditing}
            />
          ))}
        </div>
      );
